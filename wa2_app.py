    ...
    if not sp_player:
        ...
        if ENABLE_SESSION_TOPLISTS:
            ...
            with st.expander("Secret stuff"):
                ...
                _admin_pw = st.secrets.get("ADMIN_PASSWORD", "MISSING")
                st.caption(f"DEBUG: secrets key present={bool(_admin_pw and _admin_pw != 'MISSING')}, pwd_len={len(pwd)}, match={pwd == _admin_pw}")
                if pwd == _admin_pw:
